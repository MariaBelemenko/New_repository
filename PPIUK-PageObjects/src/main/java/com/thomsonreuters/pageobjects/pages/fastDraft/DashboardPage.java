...
public class DashboardPage extends AbstractPage {
    ...
    public void checkFastDraftDashboardPresents() {
        ...
        try {
            dashboard = comMethods.waitForElementToBeVisible(By.xpath(PROJECT_DASHBOARD_LABEL), 10000);
            dashboard.isDisplayed();
        } catch (NullPointerException e) {
            throw new RuntimeException("Fast Draft dashboard absents", e.getCause());
        }
    }
    ...
    public boolean isProjectPresent(String projectName) {
        ...
        try {
            project = comMethods.waitForElementToBeVisible(By.xpath(String.format(PROJECT, projectName)), 10000);
            project.isDisplayed();
        } catch (NullPointerException e) {
            return false;
        }
